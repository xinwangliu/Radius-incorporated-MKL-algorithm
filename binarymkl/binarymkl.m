function [w1,w2] = binarymkl(M,tr_label, pathfile, resultfile,EXPname,permind)
...
% permind (optional): integer that will be appended to all log files and pegasos files (to avoid conflicts
%                     among different runs on same dataset) -- is set to '0' if not passed
...
LAMBDAFILE = sprintf ('%s/lambda_%s_perm%d.file', MODELDIR, EXPname, permind);
fidlambda = fopen (LAMBDAFILE, 'w');
if (fidlambda == -1)
    fprintf ('cant open %s for writing !! \n', LAMBDAFILE);
end
lambda(1) = lambda_upper;
fprintf (fidlambda, '%.15f ', lambda(1));
i = 1;
while lambda(i)/divfact >= lambda_lower
    lambda(i+1) = lambda(i)/divfact;
    i = i+1;
    fprintf (fidlambda, '%.15f ', lambda(i));
end
fclose (fidlambda);
...
if (exist(sim_fname_fulltr_tr)~=2 || exist(sim_fname_fulltr_ve)~=2)
    ...
    write_svm_trval(sim_fname_fulltr_tr, sim_fname_fulltr_ve, M, targetmat, is_symmetric, subsample_pos,...
        subsample_neg, SUBSAMPLE, valratio);
else
    fprintf ('using already existing files %s and %s ...\n', sim_fname_fulltr_tr, sim_fname_fulltr_ve);
end
pegasos_command = sprintf ('unset LD_LIBRARY_PATH; %s -lambdaFile %s -k %d -iter %d -modelFile %s -logFile %s -testFile %s %s',...
    PEGASOSMult_EXEC, LAMBDAFILE, BATCH_SIZE, ITER, MODELFILE, PEGA_LOGFILE, sim_fname_fulltr_ve, sim_fname_fulltr_tr)
...
pegasos_command = sprintf ('unset LD_LIBRARY_PATH; %s -lambda %.9f -k %d -iter %d -modelFile %s %s > %s',...
    PEGASOS_EXEC, lambda_simacc, BATCH_SIZE, ITER, MODELFILE, sim_fname, PEGA_LOGFILE)
system (pegasos_command);
w1 = read_model(MODELFILE, num_kernels);

%% run pegasos with 'lambda' selected using cross-validation on hinge-loss of K-classifier
pegasos_command = sprintf ('unset LD_LIBRARY_PATH; %s -lambda %.9f -k %d -iter %d -modelFile %s %s > %s',...
    PEGASOS_EXEC, lambda_simloss, BATCH_SIZE, ITER, MODELFILE, sim_fname, PEGA_LOGFILE)
system(pegasos_command);
