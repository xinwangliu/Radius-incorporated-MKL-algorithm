...
void LearnReturnLast(// Input variables
  ...
  for (int i = 0; i < max_iter; ++i) {
    ...
    // Project if needed
    W.positive();  // project on positive set
...
// Read Lambda file
void ReadLambda(//input
               std::string& lambda_filename,
	       //output
	       int& num_lambda,
	       std::vector<double>& lambdaarr)
{
   std::ifstream file(lambda_filename.c_str());
   if (!file.good()) {
      std::cerr << "error reading " << lambda_filename << std::endl;
      exit(EXIT_FAILURE);
   }
   num_lambda = 0;
   while (!file.eof())  {
