function [radius,Sigma,Alpsup,w0,pos,history,obj,status] = mklMiniBall(K,yapp,C,option,verbose)

% USAGE [Sigma,Alpsup,w0,pos,history,obj,status] = mklMiniBall(K,yapp,C,option,verbose)
%
% Inputs
%
...
[radius,optimalBeta,xsup,Alpsup,w0,pos,alpha,obj] = miniBallClass([],yapp,C,lambdareg,K,Sigma,verbosesvm,alphainit);
[grad] = gradMiniBallClass(K,pos,Alpsup,radius,optimalBeta,Sigma);
...
while loop & nloopmax >0 ;
    ...
    %-----------------------------------------
    t = cputime ;
    [radius,optimalBeta,Sigma,Alpsup,w0,pos,obj] = mklMiniBallupdate(K,Sigma,pos,Alpsup,w0,C,yapp,grad,obj,radius,optimalBeta,option) ;
    %-----------------------------------------
    % Thresholding
    %-----------------------------------------
...
%     Sigma=Sigma/sum(Sigma);
    ...
    primalobj = C*sum(max( 1-yapp.*((sumK(:,pos)*Alpsup)/radius + w0)  , 0));
    primalobj= primalobj + (Alpsup'*sumK(pos,pos)*Alpsup)/(2*radius);
    %      dualitygap=(obj +   max(normek) - sum(abs(Alpsup)))/obj;
    %      %
    %      %      %if we suppose that duality gap of SVM
    %      dualitygap = (obj +   (Alpsup'*sumK(pos,pos)*Alpsup)/(2*radius) - sum(abs(Alpsup)) )/obj;
    
    dualitygap= ( primalobj - obj )/ primalobj;
