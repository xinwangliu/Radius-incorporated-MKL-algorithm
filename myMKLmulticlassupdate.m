function [Sigma,Alpsup,w0,pos,nbsv,CostNew] = myMKLmulticlassupdate(K,Sigma,pos,Alpsup,w0,C,...
    yapp,nbsv,GradNew,CostNew,option)
%------------------------------------------------------------------------------%
% Initialize
%------------------------------------------------------------------------------%
nbclass = length(unique(yapp));
d = length(Sigma);
gold = (sqrt(5)+1)/2 ;
...
switch option.algo
    case 'oneagainstall'
        %% [cost,Alpsupaux,w0aux,posaux,nbsv] = costsvmoneagainstall(K,StepSigma,DirSigma,Sigma,C,yapp)
    [costmax,Alpsupaux,w0aux,posaux,nbsvaux] = costsvmoneagainstall(K,stepmax,desc,SigmaNew,C,yapp);
    case 'oneagainstone'
...
switch option.algo
    case 'oneagainstall'
    [costmedr,Alpsupr,w0r,posr,nbsvr] = costsvmoneagainstall(K,stepmedr,desc,SigmaNew,C,yapp);
    case 'oneagainstone'
    [costmedr,Alpsupr,w0r,posr,nbsvr] = costsvmoneagainstone(K,stepmedr,desc,SigmaNew,Alpsup,C,yapp,pos,nbsv,nbclass,option) ;
end;
switch option.algo
    case 'oneagainstall'
    [costmedl,Alpsupl,w0l,posl,nbsvl] = costsvmoneagainstall(K,stepmedl,desc,SigmaNew,C,yapp);
    case 'oneagainstone'
