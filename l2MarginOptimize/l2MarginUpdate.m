function [Sigma,Alpsup,w0,pos,CostNew] = l2MarginUpdate(K,Sigma,pos,Alpsup,w0,yapp,GradNew,CostNew, option)

%------------------------------------------------------------------------------%
% Initialize
%------------------------------------------------------------------------------%

coeffMatrix = ones(size(Sigma));
coeffMatrix(1) = 0;
...
switch option.firstbasevariable
    case 'first'
        [val,coord1] = max(SigmaNew);

    case 'random'
        [val,coord1] = max(SigmaNew) ;
        coord=find(SigmaNew==val);
        indperm=randperm(length(coord1));
        coord1=coord1(indperm(1));
    case 'fullrandom'
        indzero=find(SigmaNew~=0);
        if ~isempty(indzero)
        [mini,coord1]=min(GradNew(indzero));
        coord=indzero(coord1);
    else
        [val,coord1] = max(SigmaNew) ;
    end;
        
end;
GradNew = GradNew - GradNew(coord1) ;
desc = - GradNew.* ( (SigmaNew>0) | (GradNew<0) ) ;
desc(coord1) = - sum(coeffMatrix([1:coord1-1,coord1+1:end] ).*desc([1:coord1-1,coord1+1:end]));  % NB:  GradNew(coord) = 0
...
%-----------------------------------------------------
ind = find(desc<0);
[stepmax indextt]= min(-(SigmaNew(ind))./desc(ind));
deltmax = stepmax;
...
while costmax<costmin;
    ...
    [costmax,Alpsupaux,w0aux,posaux] = costl2MarginClass(K,stepmax,desc,SigmaNew,pos,Alpsup,yapp,option);
...
%      SigmaNew=SigmaNew/sum(SigmaNew);
        ...
        desc(coord) = - sum(coeffMatrix([[1:coord-1] [coord+1:end]]).*desc([[1:coord-1] [coord+1:end]]));  
        ind = find(desc<0);
        Alpsup=Alpsupaux;
        w0=w0aux;
        pos=posaux;
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax))  & stepmax > eps;
    ...
    [costmedr,Alpsupr,w0r,posr] = costl2MarginClass(K,stepmedr,desc,SigmaNew,pos,Alpsup,yapp,option) ;
    [costmedl,Alpsupl,w0l,posl] = costl2MarginClass(K,stepmedl,desc,SigmaNew,posr,Alpsupr,yapp,option) ;
