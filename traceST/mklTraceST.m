function [trST,Sigma,Alpsup,w0,pos,history,obj,status] = mklTraceST(K,yapp,C,option,verbose)
...
[trST,trSTp,xsup,Alpsup,w0,pos,alpha,obj] = traceSTclass([],yapp,C,lambdareg,K,Sigma,verbosesvm,alphainit);
[grad] = traceSTgrad(K,pos,Alpsup,trST,trSTp,Sigma);
...
while loop & nloopmax >0 ;
    ...
    %-----------------------------------------
    t = cputime ;
    [trST,trSTp,Sigma,Alpsup,w0,pos,obj] = mklTraceSTupdate(K,Sigma,pos,Alpsup,w0,C,yapp,grad,obj,trST,trSTp,option) ;
    %-----------------------------------------
    % Thresholding
    %-----------------------------------------
...
%     format
    ...
    primalobj = C*sum(max( 1-yapp.*((sumK(:,pos)*Alpsup)/trST + w0)  , 0));
    primalobj= primalobj + (Alpsup'*sumK(pos,pos)*Alpsup)/(2*trST);
