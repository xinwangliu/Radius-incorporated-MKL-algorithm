function [radius,radiusp,Sigma,Alpsup,w0,pos,CostNew] = mklBasicRadiusupdate(K,Sigma,pos,Alpsup,w0,C,yapp,GradNew,CostNew,radiusNew,radiuspNew,option)
...
while costmax<costmin;
    ...
    %[trST,trSTp,cost,Alpsupaux,w0aux,posaux] = costtrSTclass(K0,StepSigma,DirSigma,Sigma,indsup,Alpsup,C,yapp,option)

    [radiusaux,radiuspaux,costmax,Alpsupaux,w0aux,posaux] = costBasicRadiusclass(K,stepmax,desc,SigmaNew,pos,Alpsup,C,yapp,option) ;
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax))  & stepmax > eps;
    ...
    %[trST,trSTp,cost,Alpsupaux,w0aux,posaux] = costtrSTclass(K0,StepSigma,DirSigma,Sigma,indsup,Alpsup,C,yapp,option)
    [radiusr,radiuspr,costmedr,Alpsupr,w0r,posr] = costBasicRadiusclass(K,stepmedr,desc,SigmaNew,pos,Alpsup,C,yapp,option) ;
    [radiusl,radiuspl,costmedl,Alpsupl,w01,posl] = costBasicRadiusclass(K,stepmedl,desc,SigmaNew,posr,Alpsupr,C,yapp,option) ;
