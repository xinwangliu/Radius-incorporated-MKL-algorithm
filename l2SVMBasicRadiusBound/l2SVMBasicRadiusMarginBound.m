function [Sigma,Alpsup,w0,pos,history,obj,status] = l2SVMBasicRadiusMarginBound(K,yapp,C,option,verbose)
%% Author: Xinwang Liu
% Inputs
%
...
KK0 = kerneloption.matrix;
%%compute Radius
%[radius, optimalBeta]= l2SVMradius(KK0,C,lambdareg,verbose);
%%compute Margin
%[xsup,Alpsup,w0,pos,alpha,margin] = marginClass([],yapp,C,lambdareg, kernel, kerneloption,verbosesvm,alphainit);
[radius,radiusp,xsup,Alpsup,w0,pos,alpha,obj]=l2SVMBasicRadiusClass([],yapp,C,lambdareg, K, Sigma, verbosesvm,alphainit);

%compute gradient
[grad] = l2SVMBasicRadiusMarginBoundGrad(K,pos,Alpsup,C,radiusp);
...
while loop & nloopmax >0 ;
    ...
    %-----------------------------------------
    t = cputime ;
    [radius,radiusp,Sigma,Alpsup,w0,pos,obj] = l2SVMBasicRadiusMarginBoundUpdate(K,Sigma,pos,Alpsup,w0,C,yapp,grad,obj,radius,radiusp,option) ;
    %-----------------------------------------
    % Thresholding
    %-----------------------------------------
