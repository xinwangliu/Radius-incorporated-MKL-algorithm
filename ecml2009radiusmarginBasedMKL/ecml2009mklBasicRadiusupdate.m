function [Sigma,Alpsup,w0,pos,CostNew] = ecml2009mklBasicRadiusupdate(K,Sigma,pos,Alpsup,w0,C,radiusp,yapp,GradNew,CostNew,option)
...
while costmax<costmin;
    [costmax,Alpsupaux,w0aux,posaux] = ecml2009costBasicRadiusclass(K,stepmax,desc,SigmaNew,C,radiusp,yapp,option) ;
    if costmax<costmin
        costmin = costmax;
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax))  & stepmax > eps;
    ...
    [costmedr,Alpsupr,w0r,posr] = ecml2009costBasicRadiusclass(K,stepmedr,desc,SigmaNew,C,radiusp,yapp,option) ;
    [costmedl,Alpsupl,w01,posl] = ecml2009costBasicRadiusclass(K,stepmedl,desc,SigmaNew,C,radiusp,yapp,option) ;
