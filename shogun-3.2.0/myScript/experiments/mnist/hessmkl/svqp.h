// -*- C++ -*-
// File "$Id: svqp.h,v 1.4 1998/12/16 17:03:55 leonb Exp $"
...
public:
  ...
  svreal epsgr;
  // epsgr2 -- if gradient norm < epsgr2 and instability detected
  // then no error produced
  svreal epsgr2;
...
class PrimalSVMOptim : public ConvexProgram
{
  //     Minimize          1/2 sum_{i=1}^{n-1} x_i^2
  //                    +  sum C_i phi(1-Ax_i)
  //     with  phi(u) = u + log(1+exp(-curv*u))/curv
  
public:
  // constructor/destructor
  PrimalSVMOptim(int n, int l);
  virtual ~PrimalSVMOptim();

  int l;               // Number of training points
  const svreal *a;     // Matrix of input points
  const svreal *c;     // Vector of error penalization
  svreal curv;         // Curvature of the error function
...
private:
  ...
  //
  svreal        err_func(const svreal u); 
  svreal  deriv_err_func(const svreal u); 
  svreal deriv2_err_func(const svreal u); 
};
