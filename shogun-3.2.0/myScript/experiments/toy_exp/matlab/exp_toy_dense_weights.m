function result = exp_mkl_toy_gap_SICHER
...
gaps = 10.^[-10, -1.35, -1.02, -0.7, -0.35 0.5];
ntrs = 50;
nte = 10000;
nval = 10000;
reps = 250;
...
for a=1:length(gaps)
  ...
  % Prepare mu's
  %-------------------
  block_mu = exp(-[1:gap:1+(N/d-1)*gap]');
...
if plot_gap
  ...
  clf;
  hold on;
  title(['TOY-GAP:    ntr=' sprintf('%0.4g',ntrs) '   nte=' sprintf('%0.4g',nte) , ...
  ...
  hh =legend(hs ,'L_1',['L_{', str{1}, '}'], 'L_{\infty}','bayes error');
