function [Sigma,Alpsup,w0,pos,CostNew] = l2mkltrStupdate(K,Sigma,pos,Alpsup,w0,yapp,trSTp,GradNew,CostNew,option)
...
while costmax<costmin;
    ...
    [costmax,Alpsupaux,w0aux,posaux] = costl2trStClass(K,stepmax,desc,SigmaNew,pos,Alpsup,yapp,option) ;
...
%      SigmaNew=SigmaNew/sum(SigmaNew);
        ...
        %         desc(coord) = - sum( trSTp([[1:coord-1] [coord+1:end]]).*desc([[1:coord-1] [coord+1:end]]))/(trSTp(coord));
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax))  & stepmax > eps;
    ...
    [costmedr,Alpsupr,w0r,posr] = costl2trStClass(K,stepmedr,desc,SigmaNew,pos,Alpsup,yapp,option);
    [costmedl,Alpsupl,w01,posl] = costl2trStClass(K,stepmedl,desc,SigmaNew,posr,Alpsupr,yapp,option);     
