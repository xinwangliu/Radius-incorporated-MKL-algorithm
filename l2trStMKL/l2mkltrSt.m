function [Sigma,Alpsup,w0,pos,obj,history,status] = l2mkltrSt(K,yapp,option,verbose)
...
% trSTp=ones(1,nbkernel);

if ~isfield(option,'sigmainit');
    Sigma=ones(1,nbkernel)./(trSTp*nbkernel);
else
...
[xsup,Alpsup,w0,pos,aux,obj] = l2trStClass([],yapp,lambdareg,K,Sigma,verbosesvm,alphainit);
[grad] = l2trStGrad(K,pos,Alpsup);
...
while loop & nloopmax >0 ;
    ...
    %-----------------------------------------
    t = cputime ;
    [Sigma,Alpsup,w0,pos,obj] = l2mkltrStupdate(K,Sigma,pos,Alpsup,w0,yapp,trSTp,grad,obj,option) ;
    %-----------------------------------------
    % Thresholding
    %-----------------------------------------
...
% 
%     primalobj = C* sum(max( 1 - yapp.*((sumK(:,pos)*Alpsup) + w0), 0) );
%     primalobj = primalobj + (Alpsup'*sumK(pos,pos)*Alpsup);
